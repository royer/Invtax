...
class Qif(object):
    ...
    def get_securities(self, name=None, symbol=None):
        if not name and not symbol:
            return tuple(self._securities)
        res = []
        for s in self._securities:
            valid_name = (not name or s.name == name) and True or False
            valid_symbol = (not symbol or s.symbol == symbol) and True or False
            if valid_name and valid_symbol:
                res.append(s)
        return tuple(res)
...
class Account(BaseEntry):
    ...
    def add_transaction(self, item, header=None):
        ...
        if not header:
            raise RuntimeError(six.u("no header provided yet"))
        #print("add a trans"+ str(item), file=sys.stderr)
        self._transactions[header].append(item)
