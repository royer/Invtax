...
class QifParser(object):
    ...
    def parse(cls_, file_handle, date_format=None):
        ...
        for chunk in chunks:
            ...
            if first_line == '!Type:Cat':
                ...
                raise QifParserException('Header not reconized: ' + first_line)
    ...
    def parseMemorizedTransaction(cls_, chunk, date_format=None):
        ...
        for line in lines:
            if not len(line) or line[0] == '\n' or \
                ...
                curItem.amount = cls_.currencyToFloat(line[1:])
            elif line[0] == 'C':
                ...
                split = curItem.splits[-1]
                split.amount = cls_.currencyToFloat(line[1:-1])
            else:
    ...
    def parseInvestment(cls_, chunk, date_format=None):
        ...
        for line in lines:
            if not len(line) or line[0] == '\n' or line.startswith('!Type'):
                ...
                curItem.price = cls_.currencyToFloat(line[1:])
            elif line[0] == 'Q':
                curItem.quantity = cls_.currencyToFloat(line[1:])
            elif line[0] == 'C':
                curItem.cleared = line[1:]
            elif line[0] == 'M':
                curItem.memo = line[1:]
                ...
                curItem.amount_transfer = cls_.currencyToFloat(line[1:])
            elif line[0] == 'O':
                curItem.commission = cls_.currencyToFloat(line[1:])
        return curItem
    ...
    def parseQifDateTime(cls_, qdate):
        ...
        QIF is like "7/ 9/98"  "9/ 7/99" or "10/10/99" or "10/10'01"
             or, "2/ 4' 1" for y2k or, it seems (citibankdownload 20002) 
             like "01/22/2002" or, (Paypal 2011) like "3/2/2011".  or, 
             (iBank 5.0) like "2012-03-01".
        ISO is like   YYYY-MM-DD  I think @@check
        """
        if len(qdate) == 10 and (qdate[4] == "-" or qdate[4] == "/") and (qdate[7] == "-" or qdate[7] == "/"):
            iso_date = qdate.replace("/","-")
    ...
    @classmethod
    def currencyToFloat(cls_, strcurrency):
        locale.setlocale(locale.LC_ALL,"en_US")
        if len(strcurrency) == 0:
            return 0.0
        else:
            return locale.atof(strcurrency)
        
    @classmethod
    def parseIBankAccountBalanceAmount(cls_, account,strcurrency):
        for idx in range(len(strcurrency)):
            if strcurrency[idx].isdigit():
                break
            else:
                continue
        if idx == len(strcurrency):
            raise QifParserException("Account balance has no $ symbol")
        account.currency = strcurrency[0:idx-1]
        return cls_.currencyToFloat(strcurrency[idx:])
