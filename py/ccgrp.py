...
def convertCurrencyToFloat(strCurrency):
	...
	# Don't to find $ symbol,because other foreign currency will appear
	# instead to find first digit
	r = re.search("\d",strCurrency)
	if r: 
		if strCurrency[0] == '-':
			strtemp = "-"
		else:
			strtemp = ""
		strtemp = strtemp + strCurrency[r.start():] #- will appear before currency symbol
	else:
		strtemp = "0"
...
def processOneStock (BankFile, currstock, accountname):
	...
	for line in BankFile:
		...
		if listLine[0] != currstock + u" (" + accountname + u") Total":
			...
			oneStockTotal[idxQuantityColumn] += float(listLine[idxQuantityColumn])
...
for line in iBankFile:
	...
	if len(listLine) == 1 :
		...
		if m !=None:
			currStock = m.group(1).strip()
			curr_account_name = m.group(2)
		else:
